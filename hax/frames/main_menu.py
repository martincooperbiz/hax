...
class MainMenu(Frame):
  ...
  def __init__(self, master):
    self.master = master
    super().__init__(master, bg=self.master.setting["style"]["secondary_color"])
  ...
  def init_items(self, event_func):
    ...
    for item in self.menu_items:
      btn_frame = Frame(self, bg=self.master.setting["style"]["secondary_color"])
  ...
  def mouse_hover(self, event):
    ...
    if self.current_item is not event.widget:
      event.widget["bg"] = self.master.setting["style"]["hover_color"]

  def mouse_leave(self, event):
    """Call when a mouse leave the menu item"""
    if self.current_item is not event.widget:
      event.widget["bg"] = self.master.setting["style"]["secondary_color"]

  def mouse_click(self, event, window: Windows, click_event_func):
    """Call when a mouse click the menu item"""
    self.current_window = window
    if self.current_item:
      self.current_item["bg"] = self.master.setting["style"]["secondary_color"]  # type: ignore[attr-defined]
    self.current_item = event.widget
    self.current_item["bg"] = self.master.setting["style"]["hover_color"]  # type: ignore[attr-defined]
    click_event_func(event, window)

  def create_menu_item(self, parent, text, image, window, click_event_func):
    """Create Button label"""
    if image and text:
      img = ImageTk.PhotoImage((Image.open(f"{self.master.setting['images']['path']}/{image}")).resize((30, 30)))
      btn_lbl = Label(parent, width=200, height=50, text=text, image=img, compound='left',
                      bg=self.master.setting["style"]["secondary_color"],
                      foreground=self.master.setting["style"]["forth_color"],
      ...
      btn_lbl = Label(parent, height=3, bg=self.master.setting["style"]["secondary_color"])
    return btn_lbl
