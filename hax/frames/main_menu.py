...
class MainMenu(Frame):
  ...
  def init_items(self, event_func):
    ...
    for item in self.menu_items:
      btn_frame = Frame(self, bg=self.master.config["style"]["secondary_color"])
  ...
  def mouse_hover(self, event):
    ...
    if self.current_item is not event.widget:
      event.widget["bg"] = self.master.config["style"]["hover_color"]

  def mouse_leave(self, event):
    """Call when a mouse leave the menu item"""
    if self.current_item is not event.widget:
      event.widget["bg"] = self.master.config["style"]["secondary_color"]
  ...
  def create_menu_item(self, parent, text, image, window, click_event_func):
    """Create Button label"""
    if image and text:
      img = ImageTk.PhotoImage((Image.open(f"{self.master.config['images']['path']}/{image}")).resize((30, 30)))
      btn_lbl = Label(parent, width=200, height=50, text=text, image=img, compound='left',
                      bg=self.master.config["style"]["secondary_color"],
                      foreground=self.master.config["style"]["forth_color"],
      ...
      btn_lbl = Label(parent, height=3, bg=self.master.config["style"]["secondary_color"])
    return btn_lbl
